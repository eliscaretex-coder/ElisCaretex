-- =====================================================================
-- ElisCaretex V2
-- Migration: 202608080004_production_roster_assignment_save_integrity.sql
--
-- Purpose
--   1. Make the weekly Production Roster row section authoritative for
--      normal BASE working assignments.
--   2. Preserve SUPPORT_ROLE as the explicit mixed-daily-assignment mode.
--   3. Preserve COVER as a day-level temporary role.
--   4. Align COVER authorisation with the current Staff Master capability
--      instead of rejecting an editable current-week day merely because
--      the capability was created later in the same week.
--   5. Keep save atomicity and optimistic concurrency unchanged.
--
-- Safety
--   * No published Production Roster entry is updated or deleted.
--   * No RLS/private-table privilege is weakened.
--   * Existing save remains transactionally atomic.
-- =====================================================================

begin;

create or replace function public.production_roster_resolve_display_assignment(
...
as $$
  select
    case upper(trim(coalesce(p_display_section_code, '')))
      when 'SORTING_AREA' then 'SORTING_AREA'
      when 'LABEL' then 'LABEL'
      when 'SUPERVISOR' then 'SUPERVISOR'
      when 'CLEANER' then 'CLEANER'
      when 'FINISH_TABLE_1' then case when upper(trim(coalesce(p_requested_role_code, ''))) = 'TEAM_LEADER' then 'TEAM_LEADER' else 'GENERAL_OPERATIVE' end
      when 'FINISH_TABLE_2' then case when upper(trim(coalesce(p_requested_role_code, ''))) = 'TEAM_LEADER' then 'TEAM_LEADER' else 'GENERAL_OPERATIVE' end
      when 'FINISH_TABLE_3' then case when upper(trim(coalesce(p_requested_role_code, ''))) = 'TEAM_LEADER' then 'TEAM_LEADER' else 'GENERAL_OPERATIVE' end
      when 'FINISH_OTHER' then case when upper(trim(coalesce(p_requested_role_code, ''))) = 'TEAM_LEADER' then 'TEAM_LEADER' else 'GENERAL_OPERATIVE' end
      else nullif(upper(trim(coalesce(p_requested_role_code, ''))), '')
    end,
    case upper(trim(coalesce(p_display_section_code, '')))
      when 'SORTING_AREA' then 'SORTING'
      when 'LABEL' then 'FINISH'
      when 'SUPERVISOR' then 'FINISH'
      when 'CLEANER' then 'FINISH'
      when 'FINISH_TABLE_1' then 'FINISH'
      when 'FINISH_TABLE_2' then 'FINISH'
      when 'FINISH_TABLE_3' then 'FINISH'
      when 'FINISH_OTHER' then 'FINISH'
      else null
    end,
    case upper(trim(coalesce(p_display_section_code, '')))
      when 'SORTING_AREA' then 'SORTING_MAIN'
      when 'LABEL' then 'FINISH_LABEL'
      when 'FINISH_TABLE_1' then 'FINISH_TABLE_1'
      when 'FINISH_TABLE_2' then 'FINISH_TABLE_2'
      when 'FINISH_TABLE_3' then 'FINISH_TABLE_3'
      else null
    end;
$$;

revoke all on function public.production_roster_resolve_display_assignment(text, text)
  from public, anon, authenticated;

comment on function public.production_roster_resolve_display_assignment(text, text) is
  'Internal resolver for Production Roster BASE assignments. Non-Support weekly sections drive the normal role/location; SUPPORT_ROLE remains explicit mixed-daily mode.';

create or replace function public.production_roster_cover_currently_authorised(
  p_staff_id uuid,
  p_operational_role_id uuid
)
returns boolean
language sql
stable
...
as $$
  select exists (
    ...
    where scc.staff_id = p_staff_id
      and scc.operational_role_id = p_operational_role_id
      and scc.active = true
      and scc.deleted_at is null
      and scc.effective_from <= ((now() at time zone 'Europe/Dublin')::date)
      and (
        scc.effective_until is null
        or scc.effective_until >= ((now() at time zone 'Europe/Dublin')::date)
      )
  );
$$;

revoke all on function public.production_roster_cover_currently_authorised(uuid, uuid)
  from public, anon, authenticated;

comment on function public.production_roster_cover_currently_authorised(uuid, uuid) is
  'Internal current Staff Master COVER capability check. Published history remains preserved by roster snapshots; editable current/future rosters use the capability currently enabled in Staff Master.';
...
begin
  ...
  loop
    ...
    -- The weekly row section is authoritative for normal BASE working
    -- assignments. SUPPORT_ROLE is the explicit mixed-daily-assignment mode,
    -- while COVER remains a day-level temporary assignment.
...
comment on column public.production_roster_entries.display_section_code is
  'Weekly visual/assignment section for this roster version. For BASE Working/Quality Analysis it drives the normal assignment unless the row is SUPPORT_ROLE; COVER remains day-specific.';

comment on function public.save_production_roster_week(date, text, boolean, text, jsonb, integer, text, text) is
  'Atomically saves one Morning/Evening Production Roster draft with optimistic concurrency, weekly-section/base-assignment consistency and current Staff Master COVER authorisation.';
