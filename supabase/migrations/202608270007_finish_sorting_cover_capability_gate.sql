-- A Finish-to-Sorting move is governed by Staff Master COVER capability,
-- specifically the active Sorting Area capability, not the daily assignment type.
...
begin
  ...
  if length(coalesce(p_notes,''))>1000 then raise exception using errcode='22023',message='Notes must be 1000 characters or fewer.'; end if;
  ...
  if not exists(
    select 1 from public.staff_cover_capabilities scc
    join public.operational_roles role on role.operational_role_id=scc.operational_role_id
    where scc.staff_id=p_staff_id and role.role_code='SORTING_AREA' and scc.active=true and scc.deleted_at is null
      and scc.effective_from<=v_business_date and (scc.effective_until is null or scc.effective_until>=v_business_date)
  ) then
    raise exception using errcode='42501',message='This staff member does not have an active Sorting Area COVER capability.';
  end if;

  select pre.* into v_entry from public.production_roster_entries pre join public.production_roster_versions prv on prv.roster_version_id=pre.roster_version_id
  where pre.staff_id=p_staff_id and pre.work_date=v_business_date and prv.shift_id=v_shift.shift_id and prv.status='PUBLISHED'
  ...
  where st.station_code=upper(trim(coalesce(p_from_table_code,''))) and a.area_code='FINISH' and st.active=true and st.deleted_at is null;
  ...
  if v_sorting_area_id is null or v_sorting_station_id is null then raise exception using errcode='P0002',message='Sorting Area master data is not available.'; end if;
  select * into v_finish_session from public.work_sessions ws where ws.work_date=v_business_date and ws.shift_id=v_shift.shift_id and ws.staff_id=p_staff_id and ws.area_id=v_finish_area_id and ws.status<>'CANCELLED' order by ws.updated_at desc limit 1 for update;
  if not found or v_finish_session.actual_end_at is not null then raise exception using errcode='22023',message='Confirm this staff member on the current Finish Table before moving them to Sorting.'; end if;
  if exists(select 1 from public.work_sessions ws where ws.work_date=v_business_date and ws.shift_id=v_shift.shift_id and ws.staff_id=p_staff_id and ws.area_id=v_sorting_area_id and ws.status<>'CANCELLED') then raise exception using errcode='23505',message='This staff member is already active in Sorting for this shift.'; end if;
  ...
  if v_start_at is null then
    ...
    update public.work_sessions set actual_start_at=v_start_at,updated_at=now() where work_session_id=v_finish_session.work_session_id returning * into v_finish_session;
  end if;
  ...
  select * into v_open from public.finish_staff_table_segments where work_session_id=v_finish_session.work_session_id and ended_at is null order by started_at desc limit 1 for update;
  if found then
    ...
    insert into public.finish_staff_table_segments(work_session_id,station_id,started_at,ended_at,notes,created_by_auth_user_id,updated_by_auth_user_id) values(v_finish_session.work_session_id,v_from.station_id,v_start_at,v_move_at,nullif(trim(p_notes),''),auth.uid(),auth.uid());
  end if;
  v_profile:=public.production_roster_work_profile_context(v_business_date,v_shift.shift_code);
  ...
  if v_sorting_end_at is null or v_sorting_end_at<=v_move_at then raise exception using errcode='22023',message='The roster needs an end time after the transfer before this staff member can move to Sorting.'; end if;
  ...
  update public.work_sessions set actual_end_at=v_move_at,break_minutes=v_finish_break,status='CONFIRMED',source='FINISH_TO_SORTING_TRANSFER',notes=coalesce(nullif(trim(p_notes),''),notes),updated_at=now() where work_session_id=v_finish_session.work_session_id returning * into v_saved;
  insert into public.work_sessions(production_roster_entry_id,production_roster_version_id,work_date,staff_id,area_id,station_id,shift_id,actual_start_at,actual_end_at,break_minutes,extra_non_work_minutes,adjustment_reason,status,source,confirmed_by,notes,recorded_by_auth_user_id,row_version,updated_at)
  values(v_entry.roster_entry_id,v_entry.roster_version_id,v_business_date,p_staff_id,v_sorting_area_id,v_sorting_station_id,v_shift.shift_id,v_move_at,v_sorting_end_at,v_sorting_break,0,'NONE','CONFIRMED','SORTING_MANUAL_POSITION',v_actor,concat_ws(' ',format('Finish cover transfer from %s at %s.',v_from.station_name,to_char(v_move_at at time zone 'Europe/Dublin','HH24:MI')),nullif(trim(p_notes),'')),auth.uid(),1,now()) returning work_session_id into v_sorting_session_id;
  ...
  values(auth.uid(),v_actor,'FINISH_STAFF_MOVED_TO_SORTING','work_sessions',v_saved.work_session_id::text,v_old,to_jsonb(v_saved),format('%s moved from %s to Sorting at %s%s',coalesce(v_staff_name,'Staff'),v_from.station_name,to_char(v_move_at at time zone 'Europe/Dublin','HH24:MI'),case when nullif(trim(p_notes),'') is null then '' else ': '||trim(p_notes) end),'FINISH_V4');
...
create or replace function public.get_finish_staff_context_v4(p_shift_code text default 'MORNING',p_at timestamptz default null)
returns jsonb language plpgsql stable security definer set search_path=public,auth,pg_temp as $$
declare v_base jsonb; v_staff jsonb; v_business_date date;
begin
  perform public.require_finish_production_access();
  ...
  return(v_base-'schema_version'-'source_contract'-'staff')||jsonb_build_object('schema_version','FINISH_STAFF_V4','source_contract','PUBLISHED_ROSTER_PLUS_ATTENDANCE_TABLE_SEGMENTS_AND_SORTING_COVER_CAPABILITY','staff',v_staff);
end;
$$;

comment on function public.move_finish_staff_to_sorting_v1(uuid,text,text,time,text) is
  'Moves a Finish staff member to Sorting only when Staff Master has an active Sorting Area COVER capability for the business date.';
