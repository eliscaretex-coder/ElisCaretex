-- Administrator-only account and navigation-access inventory.
...
as $$
  select auth.uid() is not null and public.has_any_role(array['ADMIN']);
$$;
...
revoke all on function public.can_view_admin_account_access() from public,anon;
grant execute on function public.get_admin_account_access_directory(text) to authenticated;
grant execute on function public.can_view_admin_account_access() to authenticated;
