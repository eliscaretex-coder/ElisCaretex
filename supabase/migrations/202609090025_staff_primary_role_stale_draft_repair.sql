-- A primary role may already have been corrected in Staff Master before the
-- current Draft is refreshed. Detect that stale snapshot as well, but never
-- change an intentional manual role whose Staff Master snapshot is current.
    ...
    select
      ...
      count(pre.roster_entry_id) filter (
        where pre.assignment_type = 'BASE'
          and pre.operational_role_id is distinct from v_new_role.operational_role_id
          and pre.staff_primary_role_code_snapshot is distinct from v_new_role.role_code
      )::integer as matching_base_count,
      count(pre.roster_entry_id) filter (
        where pre.staff_primary_role_code_snapshot is distinct from v_new_role.role_code
      )::integer as staff_snapshot_count
  ...
  if coalesce(p_apply_current_roster_role, false) then
    ...
    where rp.roster_period_id = prv.roster_period_id
      ...
      and exists (
        ...
        where pre.roster_version_id = prv.roster_version_id
          and pre.staff_id = p_staff_id
          and pre.staff_primary_role_code_snapshot = v_new_role.role_code
      );
