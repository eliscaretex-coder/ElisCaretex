-- When Finish is catching up or running ahead, keep the full relevant schedule day together.
...
begin
  ...
  select
    max(scheduled_for_date) filter (
      where scheduled_for_date<v_business_date
        and finish_contribution_count=0
        and coalesce(reconciliation_outcome_code,'')<>'PROCESSED_CONFIRMED'
        and (trolley_intake_count>0 or washed_trace_count>0)
    ),
    min(scheduled_for_date) filter (
      where scheduled_for_date>v_business_date
        and (trolley_intake_count>0 or washed_trace_count>0 or finish_contribution_count>0)
    )
  into v_continuation_date,v_advanced_date
  from physical_state;
  ...
  with candidate as (
    ...
    where pfi.product_code='CLOTHES'
      and pfi.flow_status='OPEN'
      and pfi.scheduled_for_date in (v_business_date,v_continuation_date,v_advanced_date)
  )
  ...
  return jsonb_build_object(
    ...
    'continuation_date',v_continuation_date,
