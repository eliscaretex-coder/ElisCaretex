...
drop function if exists public.save_sorting_staff_actual_v3(text,uuid,time,time,integer,text,text);

create function public.save_sorting_staff_actual_v3(
  p_shift_code text,
  p_staff_id uuid,
...
begin
  ...
  if p_keep_open then
    v_reference_end := p_actual_end_time;
  ...
  elsif p_actual_end_time is null then
    raise exception using errcode = '22023', message = 'Leaving at is required when closing the shift.';
  end if;
...
revoke all on function public.save_sorting_staff_actual_v3(text,uuid,time,time,boolean,integer,text,text)
  from public, anon, authenticated;
grant execute on function public.save_sorting_staff_actual_v3(text,uuid,time,time,boolean,integer,text,text)
  to authenticated;

comment on function public.save_sorting_staff_actual_v3(text,uuid,time,time,boolean,integer,text,text) is
  'Sorting Staff Actual editor save. p_keep_open records the start and preserves the selected leaving time only as a validation reference; it does not record a departure.';
