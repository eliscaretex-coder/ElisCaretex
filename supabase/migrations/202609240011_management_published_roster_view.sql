-- Production management can review the complete published roster. The grant
-- remains account-configurable through Accounts & Access.
...
insert into public.job_title_permission_templates(job_title_code,module_code,access_scope,can_view,can_create,can_edit,can_approve,can_manage) values
 ('PRODUCTION_SUPERVISOR','MY_ROSTER','PRODUCTION',true,false,false,false,false),
 ('PRODUCTION_MANAGER','MY_ROSTER','PRODUCTION',true,false,false,false,false),
 ('GENERAL_MANAGER','MY_ROSTER','ALL',true,false,false,false,false)
on conflict(job_title_code,module_code) do update set access_scope=excluded.access_scope,can_view=true;

insert into public.account_permission_grants(auth_user_id,module_code,access_scope,can_view,can_create,can_edit,can_approve,can_manage,granted_by_auth_user_id)
...
join public.job_title_permission_templates t on t.job_title_code=ap.job_title_code and t.module_code='MY_ROSTER'
...
declare
 ...
 v_management_view boolean:=public.has_account_permission('MY_ROSTER','VIEW','PRODUCTION');
...
begin
 ...
 if not public.has_account_permission('MY_ROSTER','VIEW',null) then raise exception using errcode='42501',message='Your account cannot view My Roster.'; end if;
 ...
 if not found and not v_management_view then raise exception using errcode='P0002',message='This account is not linked to an active Production Staff record.'; end if;
 return jsonb_build_object('generated_at',now(),'fixed_link',false,'administrator_view',v_administrator,'management_view',v_management_view,
 'staff',case when v_management_view then null else jsonb_build_object('staff_id',v_staff.staff_id,'display_name',v_staff.display_name) end,
 ...
 'weeks',coalesce((select jsonb_agg(week_json order by (week_json->>'week_start')::date) from (
   ...
   ) order by pre.work_date,pre.staff_display_name_snapshot) from public.production_roster_entries pre where pre.roster_version_id=prv.roster_version_id and (v_management_view or pre.staff_id=v_staff.staff_id)),'[]'::jsonb)) week_json
   ...
   where prv.shift_id=s.shift_id and prv.status='PUBLISHED' and rp.week_start in(v_current_week,v_current_week+7)
     and (v_management_view or exists(select 1 from public.production_roster_entries pre where pre.roster_version_id=prv.roster_version_id and pre.staff_id=v_staff.staff_id))
