-- Finish may complete customers planned for a later delivery or delivery today.
-- A customer whose delivery date has already passed must not be recorded today.
...
begin
  ...
  if p_production_business_date is null
    ...
    raise exception using errcode='22023', message='Finish trolley contribution cannot be recorded after the Delivery Date.';
  end if;
...
comment on function public.assign_finish_trolley_to_flow(text,uuid,date,text) is
  'Creates Finish trolley custody for customers delivered today or later, never after the inferred Delivery Date.';
