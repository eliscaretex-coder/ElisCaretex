...
-- ElisCaretex V2
-- Validation:
-- 202608030002_customer_schedule_history_management_validation_v2.sql
--
-- Covered:
--   - protected history RPC privileges;
--   - history metadata and selected revision read model;
...
begin
  ...
  if coalesce((v_history -> 'comparison' ->> 'has_changes')::boolean, false)
     is not true then
    raise exception 'Historical comparison did not detect the temporary change.';
  end if;
...
select jsonb_build_object(
  'status', 'PASS',
  'test', '202608030002_customer_schedule_history_management_validation_v2',
